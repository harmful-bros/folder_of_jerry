...
#define VFD_DA_H	GpioDataRegs.GPBSET.bit.GPIO48 = ON
#define VFD_DA_L	GpioDataRegs.GPBCLEAR.bit.GPIO48 = ON

#define	VFD_RS_H	GpioDataRegs.GPBSET.bit.GPIO49 = ON
#define VFD_RS_L	GpioDataRegs.GPBCLEAR.bit.GPIO49 = ON

#define VFD_CLK_H	GpioDataRegs.GPBSET.bit.GPIO50 = ON
#define VFD_CLK_L	GpioDataRegs.GPBCLEAR.bit.GPIO50 = ON

#define VFD_CE_H	GpioDataRegs.GPBSET.bit.GPIO51 = ON
#define VFD_CE_L	GpioDataRegs.GPBCLEAR.bit.GPIO51 = ON
...
volatile char g_real_str[ BUFF_SIZE ];
volatile int32 g_val_table[ BUFF_SIZE ];
volatile int32 g_vfd_cnt = 0;


static void vfd_ctl_reg_init( Uint16 ctrl_reg )
{
	int32	cnt;
...
static int32 vfd_data_load( char *buf )
{
	int32 i , x , y , z;
...
void vfd_init( void )
{
	...
	vfd_ctl_reg_init( 0x81 );	//Dout if functionally tied to Din.
	vfd_ctl_reg_init( 0x4d );	//non sleep mode & bright level adjustment.
}
...
void vfd_scroll( char *str )
{
	...
	if( flag == 0 )
	{
		memset( ( void * )g_real_str , 0x00 , sizeof( g_real_str ) );
		memset( ( void * )g_val_table , 0x00 , sizeof( g_val_table ) );

		if( str == NULL || ( length = strlen( ( const char * )str ) ) > BUFF_SIZE - 1 )  //error disposal
		{
			vfdprintf("err!");
			return;
		}
		
		sprintf( ( char * )g_real_str , "%s" , str );
	...
	vfd_data_load( ( char * )&g_real_str[ g_val_table[ g_vfd_cnt++ ] ] );
