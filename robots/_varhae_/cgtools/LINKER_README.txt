...
Starting with version 5.1.0B1, the toolset contains a new COFF linker.  This 
COFF linker executable is named lnk2000 and replaces the previous version 
of the linker.
...
3. During our alpha testing of the new linker for C6x, we have seen cases
 of users relying on undocumented old linker behavior that the new linker 
 did not mimic. For all such cases seen so far, we determined that it made
 sense to mimic the old linker behavior, and have updated the new linker.
 There is a possibility that we might find similar issues in the future. 
 We will address these issues by either updating the new linker behavior or 
 suggesting an appropriate workaround.

4. The new linker supports a sophisticated diagnostic control feature. As an
 improvement, the diagnostics format and wording are different in most cases. 
 If user is currently using scripts to parse the linker diagnostics, they
 may need to update the scripts to comprehend the changed format. The new 
 diagnostic control makes it easy to grep on diagnostic id. See 2.2 for
 more details.
...
NOT supported in the 5.0.x or older COFF linkers. 
...
following -z are passed to the linker. For example:
  ...
  cl2000 --define FOO=1 main.c -z --define BAR=2 lnk.cmd
...
options for the target (e.g. __TMS320C2000__), version (__TI_COMPILER_VERSION__), 
...
Example:
cl2000 --v --issue_remarks main.obj --run_linker --rom_model
<Linking>
lnk2000 --"tm_feedback=/tmp/10266lypVdg" hi.obj --rom_model --issue_remarks
remark: automatic RTS selection:  linking in index library "libc.a"
remark: automatic RTS selection:  linking in "rts2700.lib" in place of index
   library "libc.a"
...
cl2000 --verbose_diagnostics -g -z foo.obj bar.obj
...
task?.o*     /* matches "task1.o", "task2.obj", "taskX.o28", etc */
SECTIONS
{    
...
'--mapfile_contents=help' from the linker (lnk2000) command line (or after the 
'-z' option on the cl2000 command line), you will see the following help screen:
