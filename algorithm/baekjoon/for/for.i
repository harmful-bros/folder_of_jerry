...
int main(void)
{
 ...
 fgets(str, sizeof(str), __stdinp);
 str[strlen(str) - 1] = '\0';

 while (1)
 {
  __builtin___snprintf_chk (temp, sizeof(temp), 0, __builtin_object_size (temp, 2 > 1 ? 1 : 0), "%s", str);
  printf("%s\n", temp);
  __builtin___memmove_chk (&str[0], &str[10], strlen(str), __builtin_object_size (&str[0], 0));
